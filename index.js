...
function trader(stock, isBullTrade, onLog) {
    ...
    *****************************/
    ...
    process.on('unhandledRejection', (reason, p) => {
        log('Unhandled Rejection at: Promise', 'reason:', reason.response ? reason.response.data.message : reason.message, reason.stack);
    });
    ...
    async function start(symbol) {
        ...
        log('Angel One flight test done lastPrice on', moment(lastDay.datetime).format('YYYY-MM-DD HH:mm'), 'was', lastDay.close)
        ...
        log('Zerodha flight test done. Avaialble margin', balance, 'Max spend', usablableBalance(balance).toFixed(2), 'Collateral', parseFloat(balance - usablableBalance(balance)).toFixed(2))
        ...
        if (!(await shouldITradeToday(symbol))) {
            log(r('Kill Switch Active for morning. Not trading today'))
            return;
        }
        ...
        if (buyOrderResult.ok) {
            ...
            if (buyOrderDetails.ok) {
                ...
                log('Order Cancelled . Restarting whole thing...' + buyOrderResult.order_id)
                return //start(symbol)
            }
        }
    ...
    function shouldITradeToday(symbol) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            await startMarketWatch(undefined, (ticks, ticker) => {
                ...
                log('Should I Trade ? ', verdictUp, `(NSE ${gapUps > gapDowns} | Asia ${asianMarketOutlook})`, lastTrade, '| gapUps = ', gapUps, "| gapDowns = ", gapDowns, "| noChange = ", noChange)
